...
class Disper:
    ...
    def _options_init(self):
        ...
        self.add_option(
            ...
            % (
                os.environ.get(
                    "XDG_CONFIG_HOME", os.path.join("~", ".config", "disper")
                )
            ),
        )
        self.add_option(
        ...
        group = optparse.OptionGroup(
            self.parser, "Actions", "Select exactly one of the following actions"
        )
        self._add_option(
            group,
            "-l",
    ...
    def _add_option(self, obj, *args, **kwargs):
        ...
        if (
            sys.hexversion < 0x020500F0
            and "action" in kwargs
            and kwargs["action"] == "append_const"
        ):
    ...
    def options_parse(self, args=None):
        ...
        if "import" in self.options.actions or "export" in self.options.actions:
            if self.options.resolution:
                self.log.warning(
                    "specified resolution ignored for %s" % self.options.actions[0]
                )
            if self.options.displays:
                self.log.warning(
                    "specified displays ignored for %s" % self.options.actions[0]
                )
        ...
        if self.options.reverse_cycles is None:
            self.options.reverse_cycles = False
    ...
    def switch(self):
        ...
        if "single" in self.options.actions:
            ...
            self.log.critical(
                "program error, unrecognised action: " + ", ".join(self.options.actions)
            )
            raise SystemExit(2)
