...
class MetaModeDisplay:
    ...
    def set(self, value):
        ...
        if not r:
            raise ValueError("malformed metamode portion: %s" % value)
        self.display, dmm = r.group(1), r.group(2)
        # check display
        if not re.match(r"(DFP|CRT|TV)-\d$", self.display):
            raise Exception(
                "Garbage metamode: restart X before changing the display configuration to avoid crashes in the nVidia driver"
            )
        ...
        try:
            self.physical = res2array(self.physical)
        except:
            pass
        ...
        if len(dmmparts) > 0:
            ...
            if not r:
                raise ValueError(
                    "malformed metamode portion (bad position): %s" % value
                )
            self.position = [int(r.group(1)), int(r.group(2))]
        if len(dmmparts) > 0:
            raise ValueError(
                "malformed metamode portion (too many components): %s" % value
            )
    ...
    def __eq__(self, other):
        ...
        if not virtualS:
            virtualS = self.physical
        virtualO = other.virtual
        if not virtualO:
            virtualO = other.physical
        return (
            self.display == other.display
            and self.physical == other.physical
            and virtualS == virtualO
...
def metamode_add_extend(metamode, direction, display, physical, virtual=None):
    ...
    if physical != "NULL":
        ...
        mmdisp = MetaModeDisplay(
            "%s: %s%s %+d%+d" % (display, physical, virtual, offset[0], offset[1])
        )
    else:
...
if __name__ == "__main__":
    ...
    for mm in [
        "CRT-0: 1024x768 @1024x768 +1024+0, DFP-0: 1024x768 @1024x768 +0+0",
    ...
    for mm in [
        "DFP-9: 640x480 @640x480 +640+0, DFP-2: 123x321 @567x765 +640+0",
    ...
    for dir, mmline in [
        (
            "right",
            "CRT-0: 800x600   +0+000, DFP-0: 200x300 +800+000, TV-0: 640x480 +1000+000",
        ),
        # ('left',  'CRT-0: 800x600   +0+000, DFP-0: 200x300 -200+000, TV-0: 640x480  -840+000'),
        (
            "left",
            "CRT-0: 800x600 +840+000, DFP-0: 200x300 +640+000, TV-0: 640x480    +0+000",
        ),
        # ('top',   'CRT-0: 800x600   +0+000, DFP-0: 200x300   +0-300, TV-0: 640x480    +0-780'),
        (
            "top",
            "CRT-0: 800x600   +0+780, DFP-0: 200x300   +0+480, TV-0: 640x480    +0+000",
        ),
        (
            "bottom",
            "CRT-0: 800x600   +0+000, DFP-0: 200x300   +0+600, TV-0: 640x480    +0+900",
        ),
    ]:
