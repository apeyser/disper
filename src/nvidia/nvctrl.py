...
class NVidiaControl:
    ...
    def init_NV_CONTROL(self):
        ...
        if not NVCtrl.present:
            ...
            self.xsock = None
            raise KeyError(
                "NV-CONTROL extension not found, probably not an nVidia card"
            )
        ...
        if version[1] == 8 or version[1] == 9:
            self.xsock.close()
            raise ValueError(
                "buggy NV-CONTROL extension (arg swap bug): " + ".".join(version)
            )
    ...
    def validate_GPU_count(self):
        ...
        if self.gpucount == 0:
            raise ValueError(
                "NV-CONTROL extension found but no corresponding GPU's detected"
            )
    ...
    def query_int_attribute(self, target, displays, attr):
        ...
        rq = _NVCtrlQueryAttributeRequest(
            self.opcode, target.id(), target.type(), display_mask, attr
        )
        binrp = minx.Xchange(self.xsock, rq)
    ...
    def set_int_attribute(self, target, displays, attr, value):
        ...
        display_mask = self._displays2mask(displays)
        rq = _NVCtrlSetAttributeAndGetStatusRequest(
            self.opcode, target.id(), display_mask, attr, value
        )
        binrp = minx.Xchange(self.xsock, rq)
    ...
    def query_string_attribute(self, target, displays, attr):
        ...
        display_mask = self._displays2mask(displays)
        rq = _NVCtrlQueryStringAttributeRequest(
            self.opcode, target.id(), target.type(), display_mask, attr
        )
        binrp = minx.Xchange(self.xsock, rq)
    ...
    def set_string_attribute(self, target, displays, attr, data):
        ...
        display_mask = self._displays2mask(displays)
        rq = _NVCtrlSetStringAttributeRequest(
            self.opcode, target.id(), display_mask, attr, data
        )
        binrp = minx.Xchange(self.xsock, rq)
    ...
    def query_binary_data(self, target, displays, attr):
        """return binary data"""
        display_mask = self._displays2mask(displays)
        rq = _NVCtrlQueryBinaryDataRequest(
            self.opcode, target.id(), target.type(), display_mask, attr
        )
        binrp = minx.Xchange(self.xsock, rq)
    ...
    def query_valid_attr_values(self, target, displays, attr):
        display_mask = self._displays2mask(displays)
        rq = _NVCtrlQueryValidAttributeValuesRequest(
            self.opcode, target.id(), target.type(), display_mask, attr
        )
        binrp = minx.Xchange(self.xsock, rq)
    ...
    def get_valid_attr_values(self, target, attr):
        ...
        vv = query_valid_attr_values(target.id(), NV_CTRL_TARGET_TYPE_GPU, 0, attr)
    ...
    def string_operation(self, target, displays, attr, data):
        ...
        display_mask = self._displays2mask(displays)
        rq = _NVCtrlStringOperationRequest(
            self.opcode, target.id(), target.type(), display_mask, attr, data
        )
        binrp = minx.Xchange(self.xsock, rq)
