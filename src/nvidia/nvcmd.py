...
class NVidiaControl(NVidiaControlLowLevel):
    ...
    def get_host_architecture(target):
        ...
        ha = self.query_int_attribute(
            GPU(), NV_CTRL_TARGET_TYPE_X_SCREEN, [], NV_CTRL_ARCHITECTURE
        )
    ...
    def get_vram(self, target):
        ...
        vr = self.query_int_attribute(
            target, NV_CTRL_TARGET_TYPE_GPU, [], NV_CTRL_VIDEO_RAM
        )
        return vr.value
    ...
    def get_driver_version(self, target):
        ...
        ns = self.query_string_attribute(
            target, [], NV_CTRL_STRING_NVIDIA_DRIVER_VERSION
        )
        return ns.string
    ...
    def get_display_name(self, target, display):
        ...
        ns = self.query_string_attribute(
            target, [display], NV_CTRL_STRING_DISPLAY_DEVICE_NAME
        )
        return ns.string
    ...
    def add_metamode(self, target, mm):
        ...
        res = self.string_operation(
            target, [], NV_CTRL_STRING_OPERATION_ADD_METAMODE, str(mm)
        )
    ...
    def build_display_modepool(self, target, display, opt=None):
        ...
        Returns a boolean."""
        res = self.string_operation(
            target, [display], NV_CTRL_STRING_OPERATION_BUILD_MODEPOOL, opt
        )
        return res.flags
    ...
    def set_screen_associated_displays(self, target, displays):
        ...
        res = self.set_int_attribute(
            target, [], NV_CTRL_ASSOCIATED_DISPLAY_DEVICES, mask
        )
        return res.flags
    ...
    def get_dfp_native_resolution(self, target, display):
        ...
        res = self.query_int_attribute(
            target, [display], NV_CTRL_FLATPANEL_NATIVE_RESOLUTION
        )
        if not res.flags:
            return False
        return res.value >> 16, res.value & 0xFFFF
    ...
    def get_dfp_best_fit_resolution(self, target, display):
        ...
        This attribute is only valid for flat panel (DFP) display devices."""
        res = self.query_int_attribute(
            target, [display], NV_CTRL_FLATPANEL_BEST_FIT_RESOLUTION
        )
        if not res.flags:
            return False
        return res.value >> 16, res.value & 0xFFFF
    ...
    def set_gpu_scaling(self, target, display, starget, smethod):
        ...
        if type(smethod) == int:
            ...
            raise ValueError(
                'Scaling method must be one of "stretched", "centered" or "aspect-scaled"'
            )
    ...
    def get_xinerama_info_order(self, target):
        ...
        res = self.query_string_attribute(
            target, [], NV_CTRL_STRING_TWINVIEW_XINERAMA_INFO_ORDER
        )
        if not res.flags:
            return False
        return res.string
