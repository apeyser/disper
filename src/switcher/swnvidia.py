...
class NVidiaSwitcher:
    ...
    def import_config(self, cfg):
        ...
        if scaling:
            if len(scaling) != len(displays):
                raise Exception(
                    'number of entries in "scaling" must equal # associated displays'
                )
    ...
    def _pop_display_association(self, dorestore=True):
        ...
        if len(self._display_associations) == 0:
            raise Exception(
                "unbalanced _pop_display_assocation(), this is a program bug"
            )
    ...
    def _xrandr_switch_mod(self, mmid, virtualres):
        ...
        for i, s in enumerate(screen.get_available_sizes()):
            if s.width != virtualres[0] or s.height != virtualres[1]:
                continue
        ...
        logging.info(
            "switching to metamode %d: [%d] %s / %s" % (mmid, sizeidx, res, mmid)
        )
        screen.apply_config()
        ...
        for mm in metamodes:
            for d in mm.metamodes:
                if d.display not in displays and d.physical:
                    ...
                    if not r:
                        self.log.warning(
                            "deletion of dangling metamode %d failed" % mm.id
                        )
                    break
    ...
    def set_scaling(self, displays, scaling):
        ...
        for i, d in enumerate(displays):
            xtrainfo = ""
            curscaling = scaling
            if type(curscaling) == list:
                curscaling = curscaling[i]
            if curscaling == "default":
                try:
                    ...
                    if not sc:
                        continue
                    if (sc & 0xFFFF) == 1:
                        curscaling = "stretched"
                    elif (sc & 0xFFFF) == 2:
                        curscaling = "centered"
                    elif (sc & 0xFFFF) == 3:
                        curscaling = "aspect-scaled"
                    else:
                        self.log.warn(
                            "unrecognised scaling value for %s from nvidia-settings: %d"
                            % (d, sc)
                        )
                        continue
            ...
            if curscaling == "native":
                self.log.info(
                    "setting scaling of display %s to %s%s" % (d, curscaling, xtrainfo)
                )
    ...
    def get_scaling(self, displays):
        ...
        for d in displays:
            ...
            if not res:  # 'default' on error
                self.log.warning(
                    'could not get scaling for screen %s, reverting to "default"' % d
                )
