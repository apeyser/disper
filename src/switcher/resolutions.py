...
class Resolution:
    ...
    def set(self, val, weight=None):
        ...
        if isinstance(val, Resolution):
            if val.physical:
                self.physical = val.physical[0], val.physical[1]
            if val.virtual:
                self.virtual = val.virtual[0], val.virtual[1]
        ...
        if self.virtual:
            self.virtual = tuple(self.virtual)
    ...
    def __cmp__(self, other):
        ...
        if diff:
            return diff
        diff = (
            self.physical[0] * self.physical[1] - other.physical[0] * other.physical[1]
        )
        if diff:
            return diff
        virt1, virt2 = self.virtual, other.virtual
    ...
    def set(self, sress, displays=None):
        ...
        if isinstance(sress, ResolutionSelection):
            ...
            if len(sress) != len(displays):
                raise ValueError(
                    "number of resolutions must be equal to number of displays"
                )
...
if __name__ == "__main__":
    ...
    if rc.select() != {
        "display1": Resolution("1024x800"),
        "display2": Resolution("1000x800"),
    }:
