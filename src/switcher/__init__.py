...
class Switcher:
    ...
    def get_resolutions_display(self, disp):
        ...
        if disp in self._resolutions:
            return self._resolutions[disp]
        ...
        if len(r) == 0:
            ...
            self.log.warning(
                "no resolutions found for display %s, falling back to: %s" % (disp, r)
            )
        ...
        if edid_data:
            ...
            for d in edid.get_monitor_details():
                title, info = d
                if title != "Detailed Timing":
                    continue
                res = Resolution([info["horizontal_active"], info["vertical_active"]])
                if res in r:
                    r[r.index(res)].weight += 100
                else:
                    r.append(res)
        self.log.info(
            "resolutions of " + str(disp) + ": " + ", ".join(map(str, sorted(r)))
        )
