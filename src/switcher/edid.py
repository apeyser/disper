...
class Edid:
    ...
    def get_timing_dt(self, info):
        ...
        timing["horizontal_active"] = unpack("B", info[2])[0] + (
            (unpack("B", info[4])[0] >> 4) << 8
        )
        timing["horizontal_blanking"] = unpack("B", info[3])[0] + (
            (unpack("B", info[4])[0] & 0xF) << 8
        )

        timing["vertical_active"] = unpack("B", info[5])[0] + (
            ((unpack("B", info[7])[0] >> 4) & 0xF) << 8
        )
        timing["vertical_blanking"] = unpack("B", info[6])[0] + (
            (unpack("B", info[7])[0] & 0xF) << 8
        )

        timing["hsync_offset"] = unpack("B", info[8])[0] + (
            ((unpack("B", info[11])[0] >> 6) & 3) << 8
        )
        timing["hsync_pulse_width"] = unpack("B", info[9])[0] + (
            ((unpack("B", info[11])[0] >> 4) & 3) << 8
        )
        timing["vsync_offset"] = (unpack("B", info[10])[0] >> 4) + (
            ((unpack("B", info[11])[0] >> 2) & 3) << 8
        )
        timing["vsync_pulse_width"] = (unpack("B", info[10])[0] & 0xF) + (
            (unpack("B", info[11])[0] & 3) << 8
        )

        timing["himage_size"] = unpack("B", info[12])[0] + (
            (unpack("B", info[14])[0] >> 4) << 8
        )
        timing["vimage_size"] = unpack("B", info[13])[0] + (
            (unpack("B", info[14])[0] & 0xF) << 8
        )
