...
if __name__ == "__main__":
	nv = NVidiaSwitcher()
	# probe and list displays
	res = []
	for ndisp in range(nv.get_displays()):
		print 'display #%d: %s'%(ndisp, nv.get_display_name(ndisp))
		res.append(nv.get_display_res(ndisp))
		if len(res[-1])==0:
			print 'WARNING: no resolutions found for display, falling back to default'
			res[-1] = set(['800x600','640x480'])
	# find best common resolution; XXX only sorts in x-value
	commonres = res[0]
